 ...
 */
const showLoading: ShowLoading = function (type = TypeLoading.spinner, selector = GLOBAL_LOADING_SELECTOR) {
	const element = document.querySelector(selector) as HTMLElement || null;
	if (!element) {
		// Create loading element
		const loadingElement = document.createElement('div') as HTMLDivElement;
		switch (type) {
 ...
 */
const hideLoading: HideLoading = function (selector = GLOBAL_LOADING_SELECTOR, type = TypeClearLoading.global) {
	const onClearLoading = (element: Document | HTMLElement): void => {
